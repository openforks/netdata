...
#ifdef O_NOFOLLOW
    // Reject an existing non-regular file (e.g. a symlink or FIFO) so we never follow it
    // or truncate an unrelated file. O_TRUNC is omitted on purpose: truncation happens via
    // ftruncate() only after fstat() confirms a regular file. O_NONBLOCK avoids blocking
    // on a FIFO and O_NOFOLLOW refuses to open a symlink planted at the key path.
    if (lstat(path, &st) == 0 && !S_ISREG(st.st_mode)) {
        netdata_log_error("MCP: API key file '%s' is not a regular file, refusing to overwrite it.", path);
        return false;
    }
...
#ifdef O_NOFOLLOW
    // Refuse symlinks and special files (e.g. FIFOs that would block open()).
    if (lstat(path, &st) == 0) {
